...
function getWalletContract(signerOrProvider?: ethers.Signer | ethers.Provider) {
  const address = process.env.AGENT_WALLET_ADDRESS;
  if (!address) {
    throw new Error('AGENT_WALLET_ADDRESS not configured');
  }
  return new ethers.Contract(address, AGENT_WALLET_ABI, signerOrProvider || getProvider());
}
...
router.get('/info/:walletId', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      dailyLimit: ethers.formatEther(info[3]),
      dailyLimitRaw: info[3].toString(),
      spentToday: ethers.formatEther(info[4]),
      remainingToday: ethers.formatEther(info[5]),
      whitelistEnabled: info[6],
      active: info[7],
      totalSpent: ethers.formatEther(info[8]),
...
router.get('/id/:owner/:agentId', async (req: Request, res: Response) => {
  try {
...
router.get('/owner/:address', async (req: Request, res: Response) => {
  try {
    ...
    const wallets = await Promise.all(
      walletIds.map(async (walletId: string) => {
        const info = await contract.getWalletInfo(walletId);
        return {
          walletId,
          agentId: info[1],
...
router.get('/:walletId/allowance', async (req: Request, res: Response) => {
  try {
    const { walletId } = req.params;
...
router.get('/:walletId/recipients', async (req: Request, res: Response) => {
  try {
    const { walletId } = req.params;
...
router.get('/:walletId/operators', async (req: Request, res: Response) => {
  try {
    const { walletId } = req.params;
...
router.get('/:walletId/history', async (req: Request, res: Response) => {
  try {
    const { walletId } = req.params;
...
router.get('/:walletId/can-spend/:address', async (req: Request, res: Response) => {
  try {
...
router.post('/create', async (req: Request, res: Response) => {
  try {
    ...
    const limitWei = ethers.parseEther(dailyLimit.toString());
...
router.post('/:walletId/deposit', async (req: Request, res: Response) => {
  try {
    const { walletId } = req.params;
...
router.post('/:walletId/settings', async (req: Request, res: Response) => {
  try {
    const { walletId } = req.params;
    ...
    const signer = getSigner();
    const contract = getWalletContract(signer);
    const results: any = { walletId, updates: [] };
