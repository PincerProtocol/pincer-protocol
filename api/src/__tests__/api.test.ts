...
jest.mock('../services/blockchain', () => ({
  blockchainService: {
    ...
    getEscrow: jest.fn().mockImplementation((id: number) => {
      if (id === 1) {
...
describe('Pincer API', () => {
  ...
  describe('GET /balance/:address', () => {
    it('should return balance for valid address', async () => {
      const res = await request(app).get('/balance/0x1234567890123456789012345678901234567890');
      expect(res.status).toBe(200);
      expect(res.body.balance).toBe('1000.0');
    });

    it('should return 400 for invalid address', async () => {
      const res = await request(app).get('/balance/invalid');
  ...
  describe('GET /escrow/:txId', () => {
    ...
    it('should return 404 for non-existent escrow', async () => {
      const res = await request(app).get('/escrow/999');
      expect(res.status).toBe(404);
    });

    it('should return 400 for invalid ID', async () => {
      const res = await request(app).get('/escrow/abc');
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('POST /escrow/:txId/confirm', () => {
    it('should confirm escrow', async () => {
      const res = await request(app).post('/escrow/1/confirm');
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
  });
  ...
  describe('POST /escrow/:txId/cancel', () => {
    it('should cancel escrow', async () => {
      const res = await request(app).post('/escrow/1/cancel');
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
  });
  ...
  describe('GET /agents/:address/history', () => {
    it('should return agent history', async () => {
      const res = await request(app).get('/agents/0x1234567890123456789012345678901234567890/history');
      expect(res.status).toBe(200);
    ...
    it('should return 400 for invalid address', async () => {
      const res = await request(app).get('/agents/invalid/history');
      expect(res.status).toBe(400);
    });
  });
